...
import { Activity, AlertTriangle, Database as DatabaseIcon, RefreshCw, ShieldCheck, TrendingUp } from 'lucide-react'
...
export function SourcePerformanceDashboard({supabase}:{supabase:SupabaseClient<Database>}){
 ...
 return <section className="source-performance">
  ...
  <div className="source-kpis"><article><ShieldCheck/><small>Active sources</small><strong>{snapshot?.totals.active_sources||0}</strong></article><article><DatabaseIcon/><small>External candidates</small><strong>{(snapshot?.totals.candidate_records||0).toLocaleString()}</strong></article><article><TrendingUp/><small>Business identities</small><strong>{(snapshot?.totals.business_identities||0).toLocaleString()}</strong></article><article><Activity/><small>Publication ready</small><strong>{snapshot?.totals.publication_ready||0}</strong></article></div>
