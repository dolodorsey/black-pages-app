-- High-volume external page completion without running global classifiers after every page.
create or replace function public.black_pages_complete_external_job_fast(p_job_id uuid,p_items jsonb,p_error text default null)
...
begin
 ...
 with raw as(select value item,ordinality n from jsonb_array_elements(coalesce(p_items,'[]'::jsonb)) with ordinality),clean as(
   select left(btrim(item->>'business_name'),220) business_name,
     ...
     nullif(left(btrim(item->>'source_category'),120),'') source_category,nullif(left(btrim(item->>'source_subcategory'),120),'') source_subcategory,
...
revoke all on function public.black_pages_complete_external_job_fast(uuid,jsonb,text) from public,anon,authenticated;
grant execute on function public.black_pages_complete_external_job_fast(uuid,jsonb,text) to service_role;
