...
export function VerificationLane({supabase,city}:{supabase:SupabaseClient<Database>;city:string}){
  ...
  const filtered=useMemo(()=>{const term=query.trim().toLowerCase();return(snapshot?.identities||[]).filter(item=>item.review_tier===tier&&(!term||`${item.business_name} ${item.city} ${item.category} ${item.subcategory} ${item.source_names.join(' ')}`.toLowerCase().includes(term)))},[snapshot,tier,query])
  ...
  return <section className="verification-lane">
    <div className="verification-head"><div><span><ShieldCheck size={14}/> MASTER BUSINESS REVIEW</span><h2>Review businesses once, with every source attached.</h2><p>Duplicate discoveries are grouped into one business identity. Chamber, Restaurant Week and other evidence stays visible together. Human approval advances the private pipeline only.</p></div><button onClick={()=>void load()} disabled={loading}><RefreshCw size={15} className={loading?'spin':''}/></button></div>
    ...
    <div className="verification-kpis">
      <button className={tier==='ready'?'active':''} onClick={()=>{setTier('ready');setSelected([])}}><span>Ready</span><strong>{snapshot?.counts.ready||0}</strong><small>Reviewable source evidence</small></button>
      <button className={tier==='research'?'active':''} onClick={()=>{setTier('research');setSelected([])}}><span>Research</span><strong>{snapshot?.counts.research||0}</strong><small>Needs stronger evidence</small></button>
      <button className={tier==='hold'?'active':''} onClick={()=>{setTier('hold');setSelected([])}}><span>Hold</span><strong>{snapshot?.counts.hold||0}</strong><small>Low evidence / incomplete</small></button>
    </div>
    ...
    <div className="verification-tools"><label><Search size={14}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search business, type, city or source"/></label><button onClick={toggleAll} disabled={!visible.length}>{allVisibleSelected?'Clear batch':`Select ${Math.min(batchSize,visible.length)}`}</button></div>
    ...
    <div className="verification-list identity-list">
      {loading&&!snapshot?<div className="verification-empty">Resolving business identities and evidence bundles…</div>:visible.map(item=><article key={item.identity_id} className={selected.includes(item.identity_id)?'selected':''}>
        ...
        <div className="verification-candidate">
          <div className="verification-candidate-title"><div><strong>{item.business_name}</strong><span>{item.city}{item.state?`, ${item.state}`:''}</span></div><b>{Math.round(item.max_verification_score)}</b></div>
